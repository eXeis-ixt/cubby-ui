...
```tsx
<Switch motion="stretch" squash={false} />
```
...
```tsx
<Switch className="sm:[--switch-press-squash:0px]" motion="stretch" />
```
...
<ApiProp
  ...
  defaultValue="true"
>
  Whether the thumb flattens. Under `"default"` motion that happens on press;
  under `"stretch"` it happens mid-travel, from how far the thumb has spread.
  Off keeps the height constant in both. Sets `--switch-press-squash`, so
  `className` can still retune the depth or vary it by breakpoint.
  `SwitchVisual` defaults this to `false`.
</ApiProp>
